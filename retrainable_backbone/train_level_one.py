...
parser.add_argument('--save_eval_f', action="store", dest="save_eval_f", type=int, default=480000,help='save and eval after how many iterations')

parser.add_argument('--epoch_start', action="store", dest="epoch_start", type=int, default=1,help='train from which epoch')

parser.add_argument('--sample_noise', action="store", dest="sample_noise", type=int, default=4,help='samples noise number, 4 ')

parser.add_argument('--lambda_loss', action="store", dest="lambda_loss", type=int, default=0.2,help='0.2')
...
if not(os.path.exists(save_path)):
    os.mkdir(save_path)
if not(os.path.exists('./logs/level_one')):
    os.mkdir('./logs/level_one')


lr=input_parameters.learning_rate


def initial_motion_SkyData():
...
initial_matrix=initial_motion_SkyData()
...
for current_epoch in range(input_parameters.epoch_start-1,input_parameters.epoch_start+input_parameters.epoch_num-1):
    ...
    file = open(f"./logs/level_one/level_one-epoch-{current_epoch+1}.txt", "w")
    ...
    for iters in range(data_loader_caller.datasetSize()+1):
        ...
        if len(np.shape(input_img))<2:
          level_one_input.save_weights(save_path +'epoch_'+str(current_epoch+1)+"input_full")
          level_one_template.save_weights(save_path +'epoch_'+str(current_epoch+1)+"template_full")
          break
        ...
        if (iters * 16) % input_parameters.batch_size == 0 and iters>0:
            log = f"{iters} " \
                  f"- loss:{total_loss} " \
                  f"-ssim: {ssim_loss_total } " \
                  f"-cx-loss: {convex_loss_total}"\
                  f"-Error: {error_ave_1000/(iters+1)}"
            print(log)
            file.write(str(log) + '\n')
        ...
        if (iters * 16) % input_parameters.save_eval_f == 0 and iters > 0:

            level_one_input.save_weights(save_path +'epoch_'+str(current_epoch+1)+"input_"+str(iters))
            level_one_template.save_weights(save_path +'epoch_'+str(current_epoch+1)+"template_"+str(iters))
